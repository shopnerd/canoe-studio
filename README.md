...
- Checks: 6.1.1 wall ≤ ½ in (+⅛ tol) · 6.2.2 names visible with 4 co-ed paddlers → freeboard ≥ 1 + 5 + ¼ = 6.25 in over the lettering run · 6.6.1 swamp test · 6.6 foam only within 36 in of tips (hollow volume computed with Manifold; the foam planner on the Rules tab bisects the bulkhead distance per end so the swamped hull floats at the chosen margin, counting the concrete cap and bulkhead — `buildFoam()` in `solids.js`, checked by `node test/foam-check.mjs`; exports the foam block STL and hot-wire section templates) · Ex.5 oven-dried < 80.0 pcf, reinforcement ≤ 50% of thickness, POA ≥ 40% · 5.7.1 3-person endurance slalom beam (shear/moment, exact section I, stresses with/without FS) + ACI two-way punching · 6.2.1 no CNC/3D-printed molds (info) · Ex.8 seats ≤ 20 in.
